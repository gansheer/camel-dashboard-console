...
export function statusFilterItems(CamelApps: CamelAppKind[]): RowFilterItem[] {
    ...
    if (statuses?.length) {
        return statuses.map(version => { return { id: version, title: version }; })
    } else {
        return [];
    }
};
...
export function camelVersionFilterItems(CamelApps: CamelAppKind[]): RowFilterItem[] {
    ...
    const versions = [...new Set(CamelApps.map(app => getCamelVersions(app, 'asc')).reduce((acc, item) => {
        if (item) {
            acc.push(...item);
        }
        return acc;
    ...
    if (versions?.length) {
        return versions.map(version => { return { id: version, title: version }; })
    } else {
        return [];
    }
