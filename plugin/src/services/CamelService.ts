...
import {
  Application,
  cronjobToApplication as cronJobToApplication,
  deploymentConfigToApplication,
  deploymentToApplication,
} from '../types';
...
export async function fetchDeployments(ns: string): Promise<Application[]> {
  let deploymentsUri = ns
    ? '/api/kubernetes/apis/apps/v1/namespaces/' + ns + '/deployments'
    : '/api/kubernetes/apis/apps/v1/deployments';
  ...
  return consoleFetchJSON(deploymentsUri).then((res) => {
    return res.items.map((d: DeploymentKind) => deploymentToApplication(d));
  });
}

export async function fetchCronJobs(ns: string): Promise<Application[]> {
  let deploymentsUri = ns
    ? '/api/kubernetes/apis/batch/v1/namespaces/' + ns + '/cronjobs'
    : '/api/kubernetes/apis/batch/v1/cronjobs';
  deploymentsUri += '?labelSelector=' + METADATA_LABEL_SELECTOR_CAMEL_APP;
  console.log('>> fetchCronJobSSS url: ' + deploymentsUri);
  return consoleFetchJSON(deploymentsUri).then((res) => {
    return res.items.map((c: CronJobKind) => cronJobToApplication(c));
  });
}
...
export async function fetchDeploymentConfigs(ns: string): Promise<Application[]> {
  let deploymentConfigUri = ns
    ? '/api/kubernetes/apis/apps.openshift.io/v1/namespaces/' + ns + '/deploymentconfigs'
    : '/api/kubernetes/apis/apps.openshift.io/v1/deploymentconfigs';
  ...
  return consoleFetchJSON(deploymentConfigUri)
    .then((res) => {
      return res.items.map((d: DeploymentConfigKind) => deploymentConfigToApplication(d));
    })
    .catch((_) => {
      return null;
    });
...
async function fetchDeploymentConfig(ns: string, name: string): Promise<Application> {
  const url =
    '/api/kubernetes/apis/apps.openshift.io/v1/namespaces/' + ns + '/deploymentconfigs/' + name;
...
async function fetchCronJob(ns: string, name: string): Promise<Application> {
  return consoleFetchJSON(
    '/api/kubernetes/apis/batch/v1/namespaces/' + ns + '/cronjobs/' + name,
  ).then((res) => {
...
export async function fetchSecret(ns: string, name: string): Promise<SecretKind> {
  return consoleFetchJSON('/api/kubernetes/api/v1/namespaces/' + ns + '/secrets/' + name)
    .then((res) => {
      return res.data;
    })
    .catch((_) => {
...
export async function fetchConfigMap(ns: string, name: string): Promise<ConfigMapKind> {
  return consoleFetchJSON('/api/kubernetes/api/v1/namespaces/' + ns + '/configmaps/' + name)
    .then((res) => {
      return res.data;
    })
    .catch((_) => {
...
export async function fetchPvc(ns: string, name: string): Promise<PersistentVolumeClaimKind> {
  return consoleFetchJSON(
    '/api/kubernetes/api/v1/namespaces/' + ns + '/persistentvolumeclaims/' + name,
  )
    .then((res) => {
      return res.data;
    })
...
export async function fetchJobs(ns: string): Promise<JobKind[]> {
  return consoleFetchJSON('/api/kubernetes/apis/batch/v1/namespaces/' + ns + '/jobs/')
    .then((res) => {
      return res.items;
    })
    .catch((_) => {
...
export async function fetchJob(ns: string, name: string): Promise<JobKind> {
  return consoleFetchJSON('/api/kubernetes/apis/batch/v1/namespaces/' + ns + '/jobs/' + name)
    .then((res) => {
      return res.data;
    })
    .catch((_) => {
...
export async function populateAdddionalInfo(app: Application): Promise<Application> {
  return populateCpu(app)
    .then(populateCpuMetrics)
    .then(populateMem)
    .then(populateMemMetrics)
    .then(populateCamelExchangesTotal)
    .then(populateCamelRouteExchanges);
}
...
async function populateCpu(app: Application): Promise<Application> {
  ...
  return consoleFetchJSON(queryUrl).then((res) => {
    ...
    if (
      ...
      res.data.result[0].value.length > 1
    ) {
...
async function populateCpuMetrics(app: Application): Promise<Application> {
  ...
  return consoleFetchJSON(queryUrl)
    .then((res) => {
      const newApp: Application = { ...app };
...
export async function populateGCPauseMetrics(app: Application): Promise<Application> {
  ...
  const ns = 'namespace="' + app.metadata.namespace + '"';
  const query =
    'query=sum(jvm_gc_pause_seconds_count{' +
    ns +
    ',container="' +
    app.metadata.name +
    '"}) / (1024 * 1024)';
...
export async function populateGCOverheadMetrics(app: Application): Promise<Application> {
  const currentTimeInSeconds = Math.floor(Date.now() / 1000);
  const query = `/api/prometheus/api/v1/query_range?query=avg_over_time(jvm_gc_overhead_percent{service="${
    app.metadata.name
  }", namespace="${
    app.metadata.namespace
  }"}[1m]) * 100 / avg_over_time(system_cpu_usage[1m])&start=${
    currentTimeInSeconds - 3600
  }&end=${currentTimeInSeconds}&step=60`;
  return consoleFetchJSON(query)
    .then((res) => {
      const newApp: Application = { ...app };
...
async function populateCamelRouteExchanges(app: Application): Promise<Application> {
  const ns = 'namespace="' + app.metadata.namespace + '"';
  const query =
    'query=sum(camel_route_exchange{k8s_namespace_name="' +
    app.metadata.namespace +
    '",k8s_deployment_name="' +
    app.metadata.name +
    '"})';
...
export async function fetchApplications(ns: string): Promise<Application[]> {
  return Promise.all([fetchDeployments(ns), fetchDeploymentConfigs(ns)]).then(
...
export async function fetchApplicationPods(
  ns: string,
  applicationName: string,
): Promise<PodKind[]> {
  return consoleFetchJSON(
    '/api/kubernetes/api/v1/namespaces/' +
      ns +
      '/pods?labelSelector=app.kubernetes.io/name%3D' +
      applicationName,
  )
    .then((res) => {
      return res.items;
...
export async function fetchPodsLogs(
  ns: string,
  podName: string,
  ...
  return consoleFetchJSON(logUri)
    .then((res) => {
      return res;
    })
    .catch((_) => {
...
export async function fetchApplicationWithMetrics(
  kind: string,
  ns: string,
  name: string,
): Promise<Application> {
