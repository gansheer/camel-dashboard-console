...
import { Card, CardBody, CardTitle, Text, TextContent } from '@patternfly/react-core';
...
const ApplicationProbeHealthCard: React.FC<{ application: Application }> = ({ application }) => {
  ...
  useEffect(() => {
    if (
      ...
      application.spec.containers.length > 0
    ) {
      const container = application.spec.containers[0]; // Assuming the first container
      setProbes({
        readinessProbe: container.readinessProbe
          ? container.readinessProbe.httpGet?.path || null
          : null,
        livenessProbe: container.livenessProbe
          ? container.livenessProbe.httpGet?.path || null
          : null,
      ...
      <CardBody>
        <TextContent>
          <Text component="p">
            ...
            <Status
              title={probes.startupProbe || 'N/A'}
              status={
                probes.startupProbe &&
                application.status.availableReplicas === application.status.replicas
                  ? 'Succeeded'
                  : 'Failed'
              }
            />
          ...
          <Text component="p">
            ...
            <Status
              title={probes.readinessProbe || 'N/A'}
              status={
          ...
          <Text component="p">
            ...
            <Status
              title={probes.livenessProbe || 'N/A'}
              status={
