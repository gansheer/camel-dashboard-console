    ...
    <Card>
      ...
      <CardBody>
        <List isPlain isBordered>
          {application &&
            application.spec &&
            application.spec.initContainers &&
            application.spec.initContainers.map((container: ContainerSpec, index) => (
              <ListItem key={index}>
...
const initContainerStatus = (pods: PodKind[], initContainerName: string) => {
  const states: ContainerState[] = pods.flatMap((p) =>
    p.status.initContainerStatuses.filter((s) => s.name === initContainerName).map((s) => s.state),
  );
