  ...
  useEffect(() => {
    const sorted = [...apps].filter(filterWithSelection).sort((a, b) => {
      if (sortColumn === 'name') {
        ...
        return sortDirection === 'asc'
          ? missingReplicasA - missingReplicasB
          : missingReplicasB - missingReplicasA;
        ...
        <>
          ...
          <Table aria-label="Camel Application List">
            ...
            <Tbody>
              {sortedApplications &&
                sortedApplications
                  .filter((app) => app.metadata && app.metadata.name && app.metadata.namespace)
                  .map((app, index) => (
                    <Tr key={app.metadata.name}>
                      <Td dataLabel={columnNames.name}>
                        <Link to={applicationUrl(app)}>{app.metadata.name}</Link>
                      </Td>
                      ...
                      <Td dataLabel={columnNames.status}>
                        {(app.kind == 'Deployment' || app.kind == 'DeploymentConfig') && (
                          <Status
                            title={`${app.status.availableReplicas} of ${app.status.replicas} pods`}
                            status={
                              app.status.availableReplicas === app.status.replicas
                                ? 'Succeeded'
                                : 'Failed'
                            }
                          />
                        ...
                        {app.kind == 'CronJob' && (
                          <Status
                            title={`${app.cronStatus.lastSuccessfulTime}`}
                            status={app.cronStatus.lastSuccessfulTime ? 'Succeeded' : 'Failed'}
                          />
                        )}
                      </Td>
                      ...
                      <Td dataLabel={columnNames.camel}>
                        {app.metadata.annotations?.[METADATA_ANNOTATION_CAMEL_VERSION]}
                      </Td>
                    </Tr>
                  ))}
            </Tbody>
