...
const ApplicationDetailsCard: React.FC<{ application: Application }> = ({ application }) => {
  ...
  function getHealthStatus(application: Application): string | null {
    if (application.status) {
      return application.status.replicas === application.status.availableReplicas
        ? 'Succeeded'
        : 'Failed';
    }
    return 'Failed';
  ...
  function checkMetricsEndpointStatus(application: Application) {
    const metricsProxyUrl = (app) =>
      `/api/proxy/plugin/camel-openshift-console-plugin/service-proxy/metrics/${app.metadata.namespace}/${app.metadata.name}/`;
    consoleFetch(metricsProxyUrl(application))
      .then((res) => {
  ...
  function checkInfoEndpointStatus(application: Application) {
    const infoEndpoint = (app) =>
      `/api/proxy/plugin/camel-openshift-console-plugin/service-proxy/info/${app.metadata.namespace}/${app.metadata.name}/`;
    consoleFetchJSON(infoEndpoint(application))
      .then((res) => {
        ...
        {application ? (
          <div>
            ...
            <Card>
              ...
              <CardBody>
                <TextContent>
                  ...
                  <Status title={healthEndpoint} status={healthEndpointStatus} />
                </TextContent>
                <TextContent>
                  <strong>Metrics Endpoint:</strong>{' '}
                  <Status title={metricsEndpoint} status={metricsEndpointStatus} />
                </TextContent>
                <TextContent>
                  <strong>Info Endpoint:</strong>{' '}
                  <Status title={infoEndpoint} status={infoEndpointStatus} />
                </TextContent>
                <TextContent>
                  <strong>Prod UI Endpoint:</strong>{' '}
                  <Status title={produiEndpoint} status={produiEndpointStatus} />
                </TextContent>
              </CardBody>
            </Card>
            ...
            <Card>
              ...
              <CardBody>
                {application.metadata.annotations?.[METADATA_ANNOTATION_CAMEL_VERSION] && (
                  <TextContent>
                    <strong>Camel: </strong>{' '}
                    {application.metadata.annotations[METADATA_ANNOTATION_CAMEL_VERSION]}
                  </TextContent>
                ...
                {application.metadata.annotations?.[METADATA_ANNOTATION_CAMEL_CEQ_VERSION] && (
                  <TextContent>
                ...
                {application.metadata.annotations?.[METADATA_ANNOTATION_CAMEL_CSB_VERSION] && (
                  <TextContent>
                ...
                {application.metadata.annotations?.[
                  ...
                  <TextContent>
                    ...
                    {application.metadata.annotations[METADATA_ANNOTATION_CAMEL_SPRINGBOOT_VERSION]}
                  </TextContent>
                )}
              </CardBody>
            </Card>
