...
import { Card, CardBody, CardTitle, List, ListItem, TextContent } from '@patternfly/react-core';
    ...
    if (application && application.metadata) {
      fetchJobs(application.metadata.namespace).then((jobs) => {
        if (jobs) {
          jobs.forEach((job) => {
            if (
              job.metadata.name.startsWith(application.metadata.name) &&
              job.metadata.name.endsWith('-init')
            ) {
              newJobs.push(job);
            }
          });
